...
_bdev_compare_with_md(bool emulated)
{
	...
	char buf_miscompare[1024 + 16 /* 2 * blocklen + 2 * mdlen */];
	char md_buf[16];
	...
	memset(buf_miscompare, 0xaa, sizeof(buf_miscompare));
	/* make last md different */
	memset(buf_miscompare + 1024 + 8, 0xbb, 8);
	...
	/* 4. miscompare with md separated */
	expected_io = ut_alloc_expected_io(expected_io_type, offset, num_blocks, 0);
	TAILQ_INSERT_TAIL(&g_bdev_ut_channel->expected_io, expected_io, link);
