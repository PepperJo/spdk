 ...
 * \param cb Called when the request is complete.
 * \param cb_arg Argument passed to cb.
 * \param compare_opts Optional structure with extended IO request options. If set, this structure must be
 * valid until the IO is completed. `size` member of this structure is used for ABI compatibility and
 * must be set to sizeof(struct spdk_bdev_ext_io_opts).
 * \param write_opts Optional structure with extended IO request options. If set, this structure must be
 * valid until the IO is completed. `size` member of this structure is used for ABI compatibility and
 * must be set to sizeof(struct spdk_bdev_ext_io_opts).
 *
 * \return 0 on success. On success, the callback will always
 ...
 *   * -EBADF - desc not open for writing
 */
int spdk_bdev_comparev_and_writev_blocks_ext(struct spdk_bdev_desc *desc, struct spdk_io_channel *ch,
		struct iovec *compare_iov, int compare_iovcnt,
		struct iovec *write_iov, int write_iovcnt,
		uint64_t offset_blocks, uint64_t num_blocks,
		spdk_bdev_io_completion_cb cb, void *cb_arg,
